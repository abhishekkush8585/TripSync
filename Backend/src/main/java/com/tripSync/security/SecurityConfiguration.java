...
//configure public end points
		request.requestMatchers("/v3/api-docs/**", "/swagger-ui/**", "/auth/register", "/auth/login").permitAll()
		//in flight request from React front end
				...
				// admin | patient can check specific patient details
				.requestMatchers(HttpMethod.GET, "/patients/{userId}").hasAnyRole("ADMIN","PATIENT")
